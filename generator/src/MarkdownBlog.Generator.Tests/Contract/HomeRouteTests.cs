...
public sealed class HomeRouteTests
{
    [Fact]
    public void DistIndexHtml_Exists_AndHasMainHeading()
    {
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(templatesDir, "layout.html"), "<html><body><main>{{BODY}}</main></body></html>", Encoding.UTF8);
            ...
            var indexPath = Path.Combine(outputDir, MarkdownBlog.Generator.OutputPaths.Home);
